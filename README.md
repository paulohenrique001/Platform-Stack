...
O nome do projeto Compose das stacks opcionais fica fixo no próprio `docker-compose.yml`, igual ao nome da pasta, para o Dockge conseguir gerenciar a stack sem depender de variáveis no `.env`.

## Primeiro acesso ao Dockge

No primeiro acesso, o Dockge solicita a criação do usuário administrador pela interface.

Essas informações ficam persistidas no volume `dockge_dockge_data`. Para recriar o setup inicial, remova os volumes da stack com cuidado:
...
Se quiser gerenciar essa stack pelo Dockge, coloque o arquivo em `stacks/<nome-da-stack>/docker-compose.yml` e use o scan de stacks na interface.
...
- O arquivo `.env` é local e nas stacks opcionais deve ser usado principalmente para portas, credenciais, databases e ajustes específicos; use `.env.example` como referência pública do template.
...
- Se alguma stack opcional já foi iniciada com nome antigo `platform-*`, derrube-a pelo terminal antes de recriar com os composes atuais; containers antigos não passam a ser gerenciados automaticamente pelo Dockge.
